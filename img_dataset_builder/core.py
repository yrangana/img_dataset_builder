...
import shutil
import splitfolders
from .helper import get_dirs_inside_data_folder, check_duplicate_file_names, find_duplicate_file_names
...
def create_image_bank_folders():
    os.makedirs(os.path.join(config['image_bank_folder'], config['image_folder']), exist_ok=True)
    os.makedirs(os.path.join(config['image_bank_folder'], config['label_folder']), exist_ok=True)
...
def copy_all_images():
    if check_duplicate_file_names():
        ...
        for directory in get_dirs_inside_data_folder():
            source_dir = os.path.join(config['raw_data_folder'], directory, config['image_folder'])
            destination_dir = os.path.join(config['image_bank_folder'], config['image_folder'])
            shutil.copytree(source_dir, destination_dir, dirs_exist_ok=True)
        print("All images have been successfully copied to the image bank.")

def copy_all_labels():
    for directory in get_dirs_inside_data_folder():
        source_dir = os.path.join(config['raw_data_folder'], directory, config['label_folder'])
        destination_dir = os.path.join(config['image_bank_folder'], config['label_folder'])
        shutil.copytree(source_dir, destination_dir, dirs_exist_ok=True)
...
def split_image_bank():
    ...
    print("Image bank has been successfully split into train, validation, and test sets.")
